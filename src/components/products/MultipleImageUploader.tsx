...
export const MultipleImageUploader: React.FC<MultipleImageUploaderProps> = ({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (images.length + cropperQueue.length + files.length > maxFiles) {
      toast.error(
        `${t("products.messages.errorMaxImages")} ${maxFiles} ${t("products.messages.images")}`,
      );
      return;
    }
    ...
    const validFiles = files.filter((file) => {
      if (!acceptedFormats.includes(file.type)) {
        toast.error(
          `${t("products.messages.errorInvalidFormat")} ${file.name}`,
        );
        return false;
      }
      if (file.size > maxSizeMB * 1024 * 1024) {
        toast.error(
          `${t("products.messages.errorSizeTooLarge")} ${file.name} (${t("products.messages.max")} ${maxSizeMB}MB)`,
        );
        return false;
      }
      return true;
  ...
  const handleCropComplete = async (croppedFile: File) => {
    try {
      ...
      toast.error(
        t("common.upload.errorProcessing", "Failed to process cropped image"),
      );
    } finally {
    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="flex flex-wrap gap-4">
        {images.map((img, index) => (
          <div
            key={index}
            className="relative w-24 h-24 rounded-lg border border-border overflow-hidden group bg-input flex items-center justify-center"
          >
            <img
              src={img}
              alt={`Preview ${index}`}
